...
if (!empty($_SESSION['idAgent'])) { //pour bloque acess en retour via la touche precedent
	  ...
	  if (isset($_POST['nonvalide'])){ // nonvalide

	 
 $idCour = $_POST['idCour'];

  $sql="update courrier set etat='Attente' WHERE idCour='".$idCour."'";
  $req=$cnx->query($sql);
		...
		$reponse = $cnx->query('SELECT idCour, expCour, objetCour, date_Arriver, etat, courrier.idDepartement, courrier.idType, commentaire, nomDepartement, titre, docCour FROM courrier, departement, typecourrier WHERE courrier.idDepartement=departement.idDepartement AND courrier.idType=typecourrier.idType AND courrier.idDepartement="'.$_SESSION['idDepartement'].'"  AND courrier.etat="Valide"   ORDER BY courrier.idCour DESC');

		$courrier = "";
		while ($data = $reponse->fetch()) {

		$courrier .=' <form class="form" action="courrier_agent_valide.php" role="form" method="post"><tr><td>' .$data["idCour"]. '</td><td>' .$data["expCour"]. '</td><td>' .$data["objetCour"]. '</td><td>' .$data["date_Arriver"]. '</td><td>' .$data["nomDepartement"]. '</td><td>' .$data["etat"]. '</td><td>' .$data["docCour"]. '</td><td>' .$data["titre"]. '</td><td>' .$data["commentaire"]. '</td> <td width="1200">
						    			<a class="btn btn-default"href="view_courrier_agent.php?name='.$data["idCour"]. '"><span class="glyphicon glyphicon-eye-open"></span> Voir</a> </td>
						    		<td width="1200">	<button class="btn btn-danger"href="espace_agent.php" name="nonvalide"><span class="glyphicon glyphicon-remove"></span> NON-VALIDE</button> </td>
						    			<td><input type="hidden" name="idCour" value="'.$data['idCour'].' "> 
						    		</td> </tr> </form>' ;
		}        
