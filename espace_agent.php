...
if(isset($_POST['valide'])){
 ...
 $idCour = $_POST['idCour'];
  ...
  $sql="update courrier set etat='Valide' WHERE idCour='".$idCour."'";
  $req=$cnx->query($sql);

}

		$reponse = $cnx->query('SELECT idCour, expCour, objetCour, date_Arriver, etat, courrier.idDepartement, courrier.idType, commentaire, nomDepartement, titre, docCour FROM courrier, departement, typecourrier WHERE courrier.idDepartement=departement.idDepartement AND courrier.idType=typecourrier.idType AND courrier.idDepartement="'.$_SESSION['idDepartement'].'"  AND courrier.etat="Transferer"   ORDER BY courrier.idCour DESC');

		$courrier = "";
		while ($data = $reponse->fetch()) {

		$courrier .=' <form class="form" action="espace_agent.php" role="form" method="post"><tr><td>' .$data["idCour"]. '</td><td>' .$data["expCour"]. '</td><td>' .$data["objetCour"]. '</td><td>' .$data["date_Arriver"]. '</td><td>' .$data["nomDepartement"]. '</td><td>' .$data["etat"]. '</td><td>' .$data["docCour"]. '</td><td>' .$data["titre"]. '</td><td>' .$data["commentaire"]. '</td> <td width="1200">
						    			...
						    			<td><input type="hidden" name="idCour" value="'.$data['idCour'].' "><td width="1200"> </td> 
						    			<td width="1200">	<button name="valide" class="btn btn-success"><span class="#"></span> VALIDE</button> </td>
						    		</td> </tr> </form>' ;
		}        
