...
if (!empty($_SESSION['idAgent'])) { //pour bloque acess en retour via la touche precedent
 ...
 $idCour = $_POST['idCour'];
  ...
  $req=$cnx->query($sql);
		...
		$reponse = $cnx->query('SELECT idCour, expCour, objetCour, date_Arriver, etat, courrier.idDepartement, courrier.idType, commentaire, nomDepartement, titre, docCour FROM courrier, departement, typecourrier WHERE courrier.idDepartement=departement.idDepartement AND courrier.idType=typecourrier.idType  AND courrier.etat="Valide"   ORDER BY courrier.idCour DESC');

		$courrier = "";
		while ($data = $reponse->fetch()) {

		$courrier .=' <form class="form" action="liste_courrier_valide_admin.php" role="form" method="post"><tr><td>' .$data["idCour"]. '</td><td>' .$data["expCour"]. '</td><td>' .$data["objetCour"]. '</td><td>' .$data["date_Arriver"]. '</td><td>' .$data["nomDepartement"]. '</td><td>' .$data["etat"]. '</td><td>' .$data["docCour"]. '</td><td>' .$data["titre"]. '</td><td>' .$data["commentaire"]. '</td> <td width="1200">
